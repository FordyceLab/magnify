...
def find_buttons(image: np.ndarray,
                 num_rows: int = 46,
                 num_cols: int = 24,
    ...
    row_slope, row_intercepts = regress_clusters(points,
                                                 labels=row_labels,
                                                 num_clusters=num_rows,
                                                 ideal_num_points=num_cols)
    # We treat column indices as y and row indices as x to avoid near-infinite slopes.
    col_slope, col_intercepts = regress_clusters(points[:, ::-1],
                                                 labels=col_labels,
                                                 num_clusters=num_cols,
                                                 ideal_num_points=num_rows)
...
def cluster_1d(points: np.ndarray,
               total_length: int,
               num_clusters: int,
    ...
    spans = min(cost(i) for i in range(total_length - round(num_clusters * cluster_length)))[1]
...
def regress_clusters(points: np.ndarray,
                     labels: np.ndarray,
                     num_clusters: int,
                     ideal_num_points: int):
    ...
    for i, (y, x) in enumerate(points):
        ...
        if len(x) > 1:
            ...
            logger.warning(f"Boundary cluster only has less than 2 points."
    ...
    intercept_m, intercept_b, _, _, _ = scipy.stats.linregress(label_idxs[not_nan],
                                                               intercepts[not_nan])
